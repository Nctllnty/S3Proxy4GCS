...
-   `FLUSH_INTERVAL_MS` (Default: `-1`): Controls how quickly the read proxy (GET/HEAD) flushes data to clients. `-1` = immediate flush after each write (best TTFB); `0` = no active flushing (framework-controlled); `>0` = periodic flush at the specified millisecond interval (e.g. `20` for 20ms). Trade-off: lower values improve TTFB but increase syscalls.
...
- **Structured JSON Logging**: Native `log/slog` for modern cloud observability (Parsable JSON lines). Every slog line includes `request_id`, `source_ip`, `method`, `uri`, `status`, `duration_ms`, `content_length`, `handler`, and — when present — `access_key` (HMAC AK) and `guploader_upload_id` (GCS trace ID). A parallel CSV access-log (`pkg/reqlog`) writes the same fields plus `api_method` and `bucket` for offline analytics, and an error-only logfmt file (`pkg/errlog`, `error_YYYYMMDD.log`) mirrors every `slog.Error` for at-a-glance triage. Production default `DEBUG_LOGGING=false` emits exactly 2 log lines per data-plane request (`Received S3 Request` + `HTTP request completed`); `DEBUG_LOGGING=true` enables full header / signing traces for diagnostics only. Chi's plain-text access log middleware is intentionally disabled to avoid duplicate records.
